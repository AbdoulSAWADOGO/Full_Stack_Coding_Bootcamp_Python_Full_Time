# Création d'une classe menu manager
class MenuManager():
	def __init__(self):
		# Liste des plats fournit par l'exercice
		...
		for i in self.menu:
			men=self.menu[i]
			print(men)
	# Création de la classe pour ajout de menus
	def add_item(self,name, price, spice, gluten):
		...
		for i in self.menu:
			men=self.menu[i]
			print(men)
# Vérification de l'existance d'un plat et mise à jour
	def update_item(self,name, price, spice, gluten):
		...
		if e==0:
			...
			for i in self.menu:
				v=self.menu[i]
				print(v)
		# Suppression d'un plat après vérification
